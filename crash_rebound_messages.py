...
def rebound_watch_message(symbol: str, meta: dict, metrics: dict) -> str:
    return (
        ...
        f"📊 <b>Rebound score:</b> <b>{int(metrics.get('rebound_score', 0))}/10</b>\n"
        f"⚠️ <b>Risiko:</b> <b>{metrics.get('risk_label', 'Høy')}</b>\n\n"
        "🎯 <b>Trigger for entry:</b>\n"
        "• Brudd over intradag-high fra crash-dag\n"
        "• Volum &gt; 1.5x normal\n"
        "• Grønn dag med styrke (+3 % til +5 % eller mer)\n\n"
        "👉 Først da vurder kjøp\n\n"
        "❌ <b>Ugyldig hvis:</b>\n"
        f"• Bryter under {fmt_nok(metrics.get('invalidation_level'))}\n\n"
        "<b>Handling nå:</b>\n"
